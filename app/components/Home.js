...
class Home extends Component<Props> {
	...
	async componentDidMount() {
		...
		// const address = this.props.auth.address;
	...
	onSubmit(values) {
		...
		confirmAlert({
			...
			message: `Are you sure you want to transfer <b>${
				values.amount
			}</b> TRX To <code>${values.to}</code>`, // Message dialog
	...
	render() {
		...
		return (
			<div className={styles.container}>
				...
				<Row className="mb-4">
					<Col xs="3">
						<figure className={`${styles.box} ${styles.bandwidth}`}>
							<Row className={styles.boxRow} noGutters={true}>
								<div>
									<h3>{numberSeparator(bandwidth, ",")}</h3>
									<h6>Bandwidth</h6>
								</div>
								<div>
									<img
					...
					<Col xs="3">
						<figure className={`${styles.box} ${styles.energy}`}>
							<Row className={styles.boxRow} noGutters={true}>
								<div>
									<h3>0.0</h3>
									<h6>Energy</h6>
					...
					<Col xs="3">
						<figure className={`${styles.box} ${styles.power}`}>
							<Row className={styles.boxRow} noGutters={true}>
								<div>
									<h3>{tronWeb.fromSun(account.power)}</h3>
									<h6>Tron Power</h6>
								</div>
								<div>
									<img
					...
					<Col xs="3">
						<figure className={`${styles.box} ${styles.balance}`}>
							<Row className={styles.boxRow} noGutters={true}>
								<div>
									<h3>
										<small>
				...
				<Row className="mb-4">
					<Col xs="8">
						<figure className={`${styles.box} ${styles.dark}`}>
							<Row className="has-tabs">
								...
								<p>
									A Total of <span>{transactions.total}</span>{" "}
									Transactions
								</p>
							</Row>
					...
					<Col xs="4">
						<figure className={`${styles.box} ${styles.dark}`}>
							...
							<p className="text-sm">
								TRX can be frozen to gain TRON Power and enable
								additional features. For example, with TRON
								Power you can vote for Super Representatives.You
								can gain bandwith or energy as well. Frozen
								tokens are "locked" for a period of 3 days.
								During this period the frozen TRX cannot be
								traded. After this period you can unfreeze the
								TRX and trade the tokens. Either one of
								bandwidth or energy can be acquired by each
								freeze. You cannot acquire both resources at the
								same time. When a user unfreeze a certain
								resource, his previous votes will be completely
								voided. If a user would like to vote using the
								remaining TRON Power, he will have to perform
								his voting operations all over again.
							</p>
							<Button
								onClick={this.toggleModal.bind(this, "freeze")}
								color="danger"
							>
								FREEZE
							</Button>
					...
					{(sendTransaction.result && (
						<div className="text-center">
							...
							<a
								...
								target="_blank"
							>
								<code>{sendTransaction.transaction.txID}</code>
							</a>

							<Button
								onClick={this.toggleModal.bind(this, "send")}
								color="secondary"
							>
								Close
							</Button>
					...
					{(freezeBalance && freezeBalance.result && (
						<div className="text-center">
							...
							<a
								...
								target="_blank"
							>
								<code>{freezeBalance.transaction.txID}</code>
							</a>
